...
viewMoreBtn.classList.add("nav-link");
...
const fetchPosts = async () => {
 page++;
 try {
  const url = `https://christopher-tonnesland.no/School-projects/project_exam_site/wp-json/wp/v2/posts?_embedded`;
  const request = await fetch(url);
  const response = await request.json();
  console.log(response);

  const tag = response.map((e) => e.tags.toString().toLowerCase());
  const categories = response.map((e) => e.x_categories.toString().toLowerCase());
  ...
  if (tag == slug || categories === slug) {
   response.forEach((e) => {
    const title = e.title.rendered;
    const date = e.x_date;
    const image = e.x_featured_media;
